...
import distributionDialogUsers from '@/components/users/distributionDialogUsers'
...
export default {
  ...
  methods: {
    ...
    distributionClick (row) {
      ...
      this.dataForms.distributionForm.role_name = row.role_name
      this.dataForms.distributionForm.username = row.username
      this.dataForms.distributionForm.id=row.id

      this.$http.get('/roles').then(response => {
        if (response.data.meta.status === 200) {
          this.dataForms.distributionForm.roleList = response.data.data
        } else {
          this.$message.error(response.data.meta.msg)
        }
      }).catch(e => {
    ...
    distributionRequest(){
      this.$http.put(`/users/${this.dataForms.distributionForm.id}/role`,{
        rid:this.dataForms.distributionForm.changeRole_id
      }).then(
        response=>{
          if (response.data.meta.status === 200) {
            this.$message.success(response.data.meta.msg)
